...
github_url = config.get(
    "DEFAULT", "github_url", fallback="https://github.com/kasyan1337"
)


def open_github(event):
    webbrowser.open(github_url)  # OPEN GitHub URL when the label is clicked
...
def show_splash(duration=4000):
    try:
        ...
        try:
            ...
            ratio = min(
                max_width / original_image.width, max_height / original_image.height, 1
            )
            ...
            logging.error("Failed to load logo: %s", e)
        canvas = Canvas(splash, width=50, height=50, highlightthickness=0, bg="white")
        canvas.pack(pady=10)
        arc = canvas.create_arc(
            5, 5, 45, 45, start=0, extent=90, style=ARC, outline="#1a0dab", width=3
        )
        angle = 0
        ...
        update_arc()  # START rotating arc
        current_year = datetime.now().year
        bottom_frame = tk.Frame(splash, bg="white")
        bottom_frame.pack(side="bottom", pady=20)
        label_developed = Label(
            bottom_frame,
            text="Developed by ",
            font=("Helvetica", 10),
            bg="white",
        ...
        label_right = Label(
            bottom_frame,
            text=f" | © {current_year} All rights reserved",
            font=("Helvetica", 10),
            bg="white",
            fg="#333333",
        )
        ...
        splash.after(duration, splash.destroy)  # DESTROY splash after the duration
        splash.mainloop()
    except Exception as e:
